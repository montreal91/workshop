...
m_Button_Quit("Quit") {
    ...
    m_CheckButton.signal_toggled().connect(
        sigc::mem_fun(
            *this, &ExampleWindow::on_checkbutton_toggled
        )
    );
...
void
ExampleWindow::on_combo_position() {
...
ExampleWindow::on_adjustment1_value_changed() {
    ...
    m_VScale.set_digits((int)val); // Do static cast
