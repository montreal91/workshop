...
public:
    ...
    Vec3f operator * ( float r ) const { return Vec3f( x* r, y * r, z * r ); }
...
Vec3f crossProguct( const Vec3f& a, const Vec3f& b ) {
    return Vec3f(
        a.y * b.z - a.z * b.y,
        a.z * b.x - a.x * b.z,
        a.x * b.y - a.y * b.x
...
bool rayTriangleIntersect(
    ...
    Vec3f edge1 = v1 - v0;
    Vec3f edge2 = v2 - v0;
    Vec3f pvec = crossProguct( dir, edge2 );
    ...
    Vec3f qvec = crossProguct( tvec, edge1 );
...
public:
    MeshTriangle(
        ...
        std::memcpy( vertices.get(), verts, sizeof( Vec3f ) * maxIndex );

        vertexIndex = std::unique_ptr<uint32_t[]>( new uint32_t[ numTris * 3] );
        std::memcpy( vertexIndex.get(), vertsIndex, sizeof( uint32_t ) * numTris * 3 );

        numTriangles = numTris;
        stCoordinates = std::unique_ptr<Vec2f[]>( new Vec2f[ maxIndex ] );
        std::memcpy( stCoordinates.get(), st, sizeof( Vec2f ) * maxIndex );
    }
    ...
    void getSurfaceProperties(
        ...
        N = normalize( crossProguct( e0, e1 ) );
...
void fresnel( const Vec3f& I, const Vec3f& N, const float ior, float& kr ) {
    ...
    if ( sint >= 1 ) {
        ...
        cosi = fabsf( cosi );
        float Rs = ( ( etat * cosi ) - ( etat * cost ) ) / ( ( etat * cosi ) + ( etai * cost ) );
...
int main( int argc, char** argv ) {
    ...
    for ( int i=0; i<10; i++ ) {
        Sphere* ball1 = new Sphere( Vec3f( i * 4 - 20, 0, -15 ), 2 );
        Sphere* ball2 = new Sphere( Vec3f( i * 4 - 20, 0, -20 ), 2 );
        Sphere* ball3 = new Sphere( Vec3f( i * 4 - 20, 4, -17.5 ), 2 );

        if ( i % 2 == 0 ) {
            ball1->materialType = DIFFUSE_AND_GLOSSY;
            ball2->materialType = REFLECTION_AND_REFRACTION;
        } else {
            ball1->materialType = REFLECTION_AND_REFRACTION;
            ball2->materialType = DIFFUSE_AND_GLOSSY;
        }
        ball3->materialType = DIFFUSE_AND_GLOSSY;

        ball1->diffuseColor = Vec3f( 0.15, 0.15, 0.25 );
        ball2->diffuseColor = Vec3f( 0.25, 0.15, 0.15 );
        ball3->diffuseColor = Vec3f( 0.15, 0.25, 0.15 );

        objects.push_back( std::unique_ptr<Sphere>( ball1 ) );
        objects.push_back( std::unique_ptr<Sphere>( ball2 ) );
        objects.push_back( std::unique_ptr<Sphere>( ball3 ) );
    }
    
    Sphere *sph2 = new Sphere(Vec3f(0.5, -0.5, -8), 1.5);
    sph2->ior = 1.5;
    sph2->diffuseColor = Vec3f( 0.2, 0, 0 );
    ...
    lights.push_back(std::unique_ptr<Light>(new Light(Vec3f(0, 0, -50), 2)));
    ...
    options.height = 720;
    options.fov = 90;
    options.backgroundColor = Vec3f(0.25, 0.45, 0.25);
    options.maxDepth = 5;
