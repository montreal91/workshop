...
from .forms             import XLoginForm, XRegistrationForm, XChangePasswordForm
from .forms             import XPasswordResetRequestForm, XPasswordResetForm, XChangeEmailForm
...
def Unconfirmed():
    if current_user.is_anonimous or current_user.confirmed:
...
def PasswordResetRequest():
    ...
    form = XPasswordResetRequestForm()
    if form.validate_on_submit():
        user = XUser.query.filter_by(email=form.email.data).first()
        if user:
