...
void
BloomEffect::apply( const sf::RenderTexture& input, sf::RenderTarget& output ) {
...
void
BloomEffect::prepareTextures( sf::Vector2u size ) {
...
void
BloomEffect::filterBright( const sf::RenderTexture& input, sf::RenderTexture& output ) {
...
void
BloomEffect::blurMultipass( RenderTextureArray& renderTextures ) {
    ...
    for ( std::size_t count=0; count < 2; count++ ) {
        blur(
            renderTextures[0],
            renderTextures[1],
            sf::Vector2f( 0.0f, 1.0f / textureSize.y )
        );
        blur(
            renderTextures[1],
            renderTextures[0],
            sf::Vector2f( 1.0f / textureSize.x, 0.0f )
        );
    }
}
...
void
BloomEffect::downsample( const sf::RenderTexture& input, sf::RenderTexture& output ) {
